...
public class DeepSeekLlmGateway implements LlmGateway {
    ...
    public DeepSeekLlmGateway(
            ...
            @Value("${app.llm.base-url:https://api.deepseek.com/v1}") String baseUrl,
        ...
        if (apiKey == null || apiKey.isBlank()) {
            throw new IllegalStateException("app.llm.api-key is required for provider=deepseek");
        }
    ...
    public String chat(Long userId, String prompt) {
        if (apiKey == null || apiKey.isBlank()) {
            throw new OpenAiGatewayException("app.llm.api-key is not configured for provider=deepseek");
        }
        ...
        for (int attempt = 1; attempt <= maxAttempts; attempt++) {
            try {
                ...
                if (attempt == maxAttempts) {
                    throw new OpenAiGatewayException("DeepSeek request failed after " + maxAttempts + " attempts: " + ex.getMessage(), ex);
                }
                sleepBeforeRetry();
            }
        }
        throw new OpenAiGatewayException("DeepSeek request failed unexpectedly");
    }

    private String extractContent(ChatCompletionResponse response) {
        if (response == null || response.choices() == null || response.choices().isEmpty()) {
            throw new OpenAiGatewayException("DeepSeek returned an empty response");
        }
        ChatMessage message = response.choices().getFirst().message();
        if (message == null || message.content() == null || message.content().isBlank()) {
            throw new OpenAiGatewayException("DeepSeek returned an empty message");
        }
